"""
utils/ui_components.py — Shared UI elements and CSS for MolClass.
Avoids circular imports with app.py.
"""

import streamlit as st

# ── SVG Icon Library ──────────────────────────────────────────────────────────
def icon(name: str, size: int = 18, color: str = "#06d6a0") -> str:
    icons = {
        ...
        "settings":   f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M4.93 4.93a10 10 0 0 0 0 14.14"/><path d="M12 2v2M12 20v2M2 12h2M20 12h2"/></svg>',
        "flask":      f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 3h6M9 3v7l-5 9h16l-5-9V3"/></svg>',
        "trophy":     f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17a2 2 0 0 0 4 0v-2.34"/><path d="M6 4h12v9a6 6 0 0 1-12 0V4z"/></svg>',
        ...
        "predict":    f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>',
        ...
        "molecule":   f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="18" r="3"/><circle cx="18" cy="6" r="3"/><circle cx="18" cy="18" r="3"/><line x1="6" y1="15" x2="18" y2="9"/><line x1="18" y1="15" x2="18" y2="9"/></svg>',
        ...
        "pkg":        f'<svg width="{size}" height="{size}" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12.89 1.45l8 4A2 2 0 0 1 22 7.24v9.53a2 2 0 0 1-1.11 1.79l-8 4a2 2 0 0 1-1.79 0l-8-4a2 2 0 0 1-1.1-1.8V7.24a2 2 0 0 1 1.11-1.79l8-4a2 2 0 0 1 1.78 0z"/><polyline points="2.32 6.16 12 11 21.68 6.16"/><line x1="12" y1="22.76" x2="12" y2="11"/></svg>',
...
def section_header(icon_name: str, title: str, subtitle: str = "", icon_bg: str = "var(--teal-dim)", icon_color: str = "#06d6a0"):
    sub_html = f'<p>{subtitle}</p>' if subtitle else ""
    st.markdown(f"""
    <div class="section-header">
      <div class="section-header-icon" style="background:{icon_bg};">
        {icon(icon_name, 18, icon_color)}
      </div>
      <div>
        <h2>{title}</h2>
        {sub_html}
...
def step_info_bar(step_num: str, title: str, desc: str):
    st.markdown(f"""
    <div class="step-info-bar">
      <span class="step-info-bar-num">STEP {step_num}</span>
      <div class="step-info-bar-text"><strong>{title}</strong> — {desc}</div>
    </div>
    """, unsafe_allow_html=True)
